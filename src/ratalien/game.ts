...
export class GameField extends Control {
  ...
  constructor(parentNode: HTMLElement) {
    ...
    canvas.node.onmousedown = e => {
      ...
      let listener = () => {
        const startX = this.multiStart.x >= this.cursor.x ? this.cursor.x : this.multiStart.x
        const finishX = this.multiStart.x < this.cursor.x ? this.cursor.x : this.multiStart.x
        const startY = this.multiStart.y >= this.cursor.y ? this.cursor.y : this.multiStart.y
        const finishY = this.multiStart.y < this.cursor.y ? this.cursor.y : this.multiStart.y
        this.multiStart = null;
        const unitsMiltiArray = this.units.filter(un => {
          if (un.position.x >= startX && un.position.x < finishX
              && un.position.y >= startY && un.position.y < finishY
              && un.player === 0) {
            console.log('PLAYER', un.player)
            return un
          }
          // *******rect coords
        })
        unitsMiltiArray.forEach(u => u.makeActive())
        // console.log('MULTIiii',unitsMiltiArray)
        this.traceMap.activeMultiUnitsCoordinates(unitsMiltiArray)
        this.traceMap.getActiveUnits().forEach(unit => {
          //TODO финальная точка определяется то место на котором закончилось выделение
          //а должно быть по отдельному клику
    ...
    canvas.node.onclick = e => {
      ...
      //todo если есть активные юниты тогда при клике по полю- setMode
      console.log(this.traceMap.isMultiUnits())
      // if(this.traceMap.isMultiUnits()){
      ...
      if (this.mode == 0) {
        ...
        console.log(this.mode, 'MODE')
        //TODO if multiselect this utems mode===2
        this.mode = 0;
        this.traceMap.setPathFinishPoint({x: cursorTile.x, y: cursorTile.y})
        this.selectedUnit.makeActive()
...
//возможно, тут лучше передвать не нейм, а сам объект созданного солдата? 
  addUnit(player:number, name:string){
    ...
    unit.onClick = () => {
      ...
      this.mode = 2;
      //TODO unitOnMultisecect mode===2
    }
    unit.onMouseEnter = () => {
      this.hoveredUnit.push(unit);
      //  console.log(this.hoveredUnit, 'HOVERED')
    }
    unit.onMouseLeave = () => {
  ...
  // unitClickHandler(unit: UnitObject) {
  //   const cursorTile = this.getTileCursor();
  //  // unit.clearStepIndex()
  //   this.traceMap.clearData()
  //   this.traceMap.activeUnitCoordinates(unit, cursorTile)
